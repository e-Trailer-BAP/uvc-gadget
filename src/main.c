...
void process_frame(unsigned char *data_pointer, int width, int height) {
    cv::Mat frame(height, width, CV_8UC3, data_pointer);
    cv::Mat processed_frame;
    cv::cvtColor(frame, processed_frame, cv::COLOR_BGR2GRAY);
    memcpy(data_pointer, processed_frame.data, processed_frame.total() * processed_frame.elemSize());
}
...
int main(int argc, char *argv[])
{
    ...
    while (capture_frames) {
        // Capture frame into data_pointer
        // ...

        // Process the captured frame
        process_frame(data_pointer, frame_width, frame_height);
